...
import ru.example.cus.support.Mailpit;
...
class EndToEndScenarioIT extends AbstractIntegrationTest {
    ...
    @Autowired
    private ru.example.cus.iam.application.OperatorSettingsService settings;
    ...
    void the_whole_chain_from_a_draft_form_to_the_expiry_email_works() {
        ...
        try (WebhookStub consumer = new WebhookStub(200)) {
            ...
            assertThat(delivered)
                    .anySatisfy(request ->
                            assertThat(request.headers().get("x-cus-event")).isEqualTo(EventTypes.CONSENT_REVOKED));
            assertThat(delivered.get(0).headers()).containsKey("x-cus-signature");
