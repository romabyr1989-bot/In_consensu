...
import ru.example.inconsensu.notification.application.WebhookSender;
...
class WebhookDeliveryIT extends AbstractIntegrationTest {
    ...
    void granted_and_revoked_events_reach_the_subscriber_in_order_and_are_signed() {
        try (WebhookStub stub = new WebhookStub(200)) {
            ...
            List<String> events = received.stream()
                    .map(request -> request.headers().get("x-inconsensu-event"))
                    .toList();
            ...
            assertThat(first.headers()).containsKey("x-inconsensu-delivery-id");
            assertThat(first.headers()).containsEntry("x-tenant", "cus");
            assertThat(first.headers().get("x-inconsensu-signature"))
            ...
            String revoked = received.stream()
                    .filter(request ->
                            EventTypes.CONSENT_REVOKED.equals(request.headers().get("x-inconsensu-event")))
                    .findFirst()
                    .orElseThrow()
                    .body();
    ...
    void test_send_reaches_the_subscriber_without_creating_an_event() {
        try (WebhookStub stub = new WebhookStub(204)) {
            ...
            assertThat(stub.received().get(0).headers()).containsEntry("x-inconsensu-event", "test.ping");
