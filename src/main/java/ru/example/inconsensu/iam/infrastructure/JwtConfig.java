...
public class JwtConfig {
    ...
    @Bean
    public SecretKey jwtSecretKey(InConsensuProperties properties) {
        String configured = properties.security().jwt().secret();
        if (configured == null || configured.isBlank()) {
            LOG.warn("inconsensu.security.jwt.secret не задан: сгенерирован временный ключ. "
                    + "Выданные токены станут недействительными при перезапуске и не будут приниматься "
                    + "другими экземплярами. Для эксплуатации задайте INCONSENSU_JWT_SECRET (NFR-3).");
        ...
        if (secret.length < MINIMUM_SECRET_BYTES) {
            throw new IllegalStateException("inconsensu.security.jwt.secret короче " + MINIMUM_SECRET_BYTES
