...
public final class FormRequisitesValidator {
    ...
    public static FormValidationResult validate(FormValidationInput input) {
        ...
        for (int index = 0; index < items.size(); index++) {
            ...
            // FR-1.4, предупреждение: «специальные категории включены в общий пункт» — то есть смешаны с
            // обычными внутри одного пункта. Прежнее условие срабатывало на числе пунктов формы и потому
            // ругалось как раз на правильное оформление, когда их вынесли отдельно, а смешение пропускало.
            if (item.mixedPdnCategories()) {
                warnings.add(FormValidationResult.Finding.item(
