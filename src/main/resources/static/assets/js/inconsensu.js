...
(function () {
    ...
    document.body.addEventListener('htmx:afterSwap', function (event) {
        if (event.detail.target && event.detail.target.id === 'ic-modal-content') {
            var modalElement = document.getElementById('ic-modal');
            var modal = bootstrap.Modal.getOrCreateInstance(modalElement);
            modal.show();
        }
        if (event.detail.target && event.detail.target.id === 'card-body') {
            var openModal = bootstrap.Modal.getInstance(document.getElementById('ic-modal'));
            if (openModal) {
                openModal.hide();
            }
        }
