...
Интерфейс сотрудника переписан на Angular 19 + Angular Material (ADR-0087, отступление от UI-0.2 по
решению заказчика). Новое приложение открывается на `/app/`, прежние страницы Thymeleaf — на `/ui/`. Обе
версии работают параллельно: приёмка §16 пока держится на страницах Thymeleaf, и они остаются до тех пор,
пока проверки не переедут на новое приложение.

Перенесены все разделы §16: главная, поиск клиента и карточка с каналами, передачами и лентой событий,
досье согласия, отзыв, каталог (типы, формы, конструктор, согласование, версия), импорт, уведомления,
webhooks, аудит, пользователи и настройки оператора.
