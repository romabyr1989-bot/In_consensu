...
Интерфейс сотрудника переписывается на Angular 19 + Angular Material (ADR-0087, отступление от UI-0.2 по
решению заказчика). Новое приложение открывается на `/app/`, прежние страницы Thymeleaf — на `/ui/`, они
работают параллельно, пока экраны переносятся по одному. Перенесены: каркас с меню по ролям, главная,
справочник третьих лиц.
