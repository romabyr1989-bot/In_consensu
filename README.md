...
`scripts/demo.sh` проходит сценарий §11 целиком и сам за собой убирает: форму, клиента и согласие он
заводит на каждом прогоне, отзыв делает по своему клиенту. Запускать можно сколько угодно раз подряд;
демо-данные при этом остаются нетронутыми. Если база всё же понадобится чистой — `make db-reset` и
перезапуск `make up`.
...
## Рабочее место на Angular

Интерфейс сотрудника — одностраничное приложение на Angular 19 + Angular Material (ADR-0087, отступление
от UI-0.2 по решению заказчика). Оно открывается на `/app/`; вход и сохранённые ссылки на `/ui/` туда же и
уводят. Прежние экраны на Thymeleaf удалены вместе с тестами их разметки (ADR-0089); на `/ui/` остались
только страницы, у которых нет замены в приложении: вход, отказ в доступе, истёкшая сессия и
самообслуживание клиента `/self/ui/**` (UI-18) — оно открывается по одноразовой ссылке, без учётной записи.

Разделы §16: главная, клиенты (список и поиск в одном месте) с карточкой, каналами, передачами и лентой
событий, досье согласия, отзыв, каталог (типы, формы, конструктор, согласование, версия), импорт,
уведомления, webhooks, аудит, пользователи и настройки оператора.

Приёмка §16 держится на слое данных: `src/test/java/ru/example/inconsensu/ui/Workplace*ApiIT.java`
проверяют права ролей, маскирование контактов, запись в журнал доступа к ПДн, кворум согласования и
безопасные умолчания — то есть правила сервера, которые от смены интерфейса не изменились.

Данные приложение берёт из `/ui/api` — это внутренний слой интерфейса, а не опубликованный контракт §9:
сотрудник работает по серверной сессии с CSRF, и токен в JavaScript не выносится (UI-0.3). Поэтому
`/ui/api` не входит в `docs/openapi.yaml` и меняется вместе с экранами.

Исходники — `src/main/frontend/inconsensu-ui`, собранная статика лежит в `src/main/resources/static/app`
и попадает в JAR, поэтому `./mvnw verify` работает без Node. Пересобрать фронт: `make ui` (нужен Node 22
и доступ к npm); во время работы над интерфейсом удобнее `make ui-dev` — Angular на :4200 с проксированием
запросов на :8080.
