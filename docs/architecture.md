...
## 2а. Интерфейс сотрудника

Рабочее место — одностраничное приложение на Angular (ADR-0087), собранная статика лежит в артефакте и
отдаётся с `/app/**`. Данные оно берёт из `/ui/api/**` — внутреннего слоя интерфейса поверх тех же
сервисов представлений, что и раньше: сотрудник работает по серверной сессии с CSRF, токен в JavaScript
не выносится (UI-0.3). Машинная цепочка `/api/v1/**` для интерфейса не используется и остаётся контрактом
для внешних систем (§9).

На сервере страницами остались только вход, отказ в доступе, истёкшая сессия и самообслуживание клиента
`/self/ui/**`: у последнего нет учётной записи, оно открывается по одноразовой ссылке (UI-18, ADR-0089).
...
```mermaid
flowchart TB
  ui["ui<br/>слой данных /ui/api + вход и страница клиента"]
