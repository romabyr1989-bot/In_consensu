...
Проверьте заодно, включена ли вторая линия защиты: `SELECT has_table_privilege('<роль службы>',
'audit_event', 'UPDATE');` обязан вернуть `false`. `true` означает, что установка идёт под одной ролью и
права не отзывались, — порядок перехода на две роли описан в
[`install.md`](install.md#вторая-линия-защиты-журналов-fr-102).
...
| `inconsensu.retention.notifications` | `P1Y` | срок хранения журнала уведомлений |
